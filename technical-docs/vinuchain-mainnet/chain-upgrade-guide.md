...
Do not add `--port`, `--http`, `--ws`, `--nat`, or new bootnodes during the
cutover. Do not change `--datadir` or the working directory. If the old command
omitted `--datadir` and both effective configurations matched, keep it omitted.
If pre-flight required the explicit datadir pin, use that one recorded change.

...
- **Mainnet begins its move to testnet feature parity on 2026-08-29 at 10:00 UTC** — see the [Mainnet Upgrade Guide (ELEMONT)](../vinuchain-mainnet/chain-upgrade-guide.md). Until the first activation seal, mainnet runs the pre-ELEMONT rule set (`Berlin`, `London`, `Llr`, `Podgorica`) and the **V1** SFC contract. The release activates Shanghai, Cancun, Prague, SfcV2, Elemont, ElemontPubkeyValidation, `VinuBLS12381`, `VinuLatestEVM`, and `PaybackV2` — the latter repointing `Economy.QuotaCacheAddress` off the V1 Quota proxy `0x1c4269fbbd4a8254f69383eef6af720bcd0acda6` onto a freshly deployed `QuotaContractV2`. Because the EVM forks stage sequentially, this crosses **five consecutive epoch seals** and can take up to roughly 20 hours after the swap. The `SfcV2Patch*` and `PaybackV2Patch` flags stay false on mainnet: they only re-flash bytecode on a chain that already activated with stale bytecode, and mainnet's first SfcV2 activation installs the identical Cycle-165 blob directly.
...
| ------- | ---------- | -------------------------------- | --------------- |
| Mainnet | 207 (0xcf) | `https://rpc.vinuchain.org`      | **Feature-parity upgrade begins 2026-08-29 10:00 UTC.** Active as checked 2026-08-25: Berlin, London, Llr, Podgorica; SFC **V1**; Quota proxy `0x1c4269fb…0acda6`. Across five consecutive seals it gains Shanghai, Cancun, Prague, SfcV2 (+30% base-fee burn), Elemont, ElemontPubkeyValidation, VinuBLS12381, VinuLatestEVM, and PaybackV2 (Quota address changes). Only the `SfcV2Patch*` / `PaybackV2Patch` re-flash flags stay unset |
...
INFO Backfilled SFC V2 mainnet delegations block=<N> appended=<EXPECTED_APPENDED> repaired=<EXPECTED_REPAIRED>
```
...
| Mainnet seal-time logs (ELEMONT SfcV2 activation)         | 1× `Applying SFC V2 bytecode upgrade …`; compare both state-dependent `appended` and `repaired` fields with the near-activation audit; verify the compiled pair count separately   |
...
**As checked 2026-08-25, mainnet (chain 207) remained pre-ELEMONT.** Berlin, London, Llr, and Podgorica were active; SfcV2 and the later EVM flags were not. `Economy.QuotaCacheAddress` still pointed at the live mainnet V1 Quota proxy `0x1c4269fbbd4a8254f69383eef6af720bcd0acda6`. Confirm the live state directly against the public RPC:
...
`PaybackV2`, `VinuBLS12381`, and `VinuLatestEVM` report `false` on mainnet **until the 2026-08-29 ELEMONT upgrade**, which activates all three (see the [Mainnet Upgrade Guide](../vinuchain-mainnet/chain-upgrade-guide.md)). They arrive at different seals: `PaybackV2` at seal 1, `VinuBLS12381` at seal 4, `VinuLatestEVM` at seal 5. SFC `version()` returns `0x333035` (`"305"`) once SfcV2 is active.

...
The latest public recovery snapshot is the post-`SfcV2Patch8` object (`v2.0.43-elemont`, sealed 2026-07-08 at block 1,529,204 / epoch 6118):

```text
https://vinu-blockchain-genesis.s3.amazonaws.com/chaindata-snapshots/testnet-chaindata-v2.0.43-elemont-20260708T115942Z-clean.tar.gz
```

Snapshot SHA256:

```text
ddfe4ce3f32041d72581ce4c286800547ed0f091f2b39ad3f19c8c1a9be79f79
```

It was produced under the `20260708T115942Z` object name; `SNAPSHOT_INFO.txt` records snapshot timestamp `2026-07-08T11:59:42Z`, tip block `1,529,204`, and epoch `6,118`, with `SfcV2Patch7` and `SfcV2Patch8` (plus `VinuBLS12381`, `VinuLatestEVM`, `PaybackV2`) all sealed. A stale snapshot (pre-`SfcV2Patch8`) will replay historical forks under the wrong rule set and hit `wrong event epoch hash`; always use the current object above.
...
| Testnet | 206 (0xce) | `https://vinufoundation-rpc.com` | v2.0.44-elemont deployed; the above plus PaybackV2/BLS/latest-EVM active, SfcV2Patch7 + SfcV2Patch8 (self-service `reactivateValidator`) active, SfcV2Patch9 (reward-cursor over-mint + repeated-reactivation fixes) deployed and activating at the next epoch seal; Quota proxy `0x89D1cBD9…29e4` |
...
{% hint style="info" %}
**Dependency pins.** `v2.0.44-elemont` uses go-vinu `v1.20.25-quota` (the `v1.20.24-quota` precompile/ModExp/EVM-fork surface — Shanghai, selected Cancun execution support, Prague/EIP-7702 set-code transactions, VinuBLS12381, VinuLatestEVM — plus the cherry-picked CVE-2023-40591 p2p ping-flood goroutine bound) and lachesis-base `v0.1.6-elemont`, unchanged from v2.0.41 (the SfcV2Patch8 and SfcV2Patch9 changes are SFC-bytecode-only, no dependency bump). `make opera` fetches dependencies on first build.
{% endhint %}
{% endstep %}
